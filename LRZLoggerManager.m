...
#import "LRZLoggerManager.h"
// 日志保留最大天数
static const int LogMaxSaveDay = 7;
// 日志文件保存目录
static const NSString* LogFilePath = @"/Documents/CIBNLog/";
@interface LRZLoggerManager()
...
@implementation LRZLoggerManager
/**
 *  获取单例实例
 *
 *  @return 单例实例
...
+ (instancetype) sharedInstance{
    ...
    static LRZLoggerManager* instance = nil;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!instance) {
            instance = [[LRZLoggerManager alloc]init];
            //自己建立一个线程
            instance.concurrentWriteFileQueue = dispatch_queue_create("LRZWriteFileQueue", DISPATCH_QUEUE_CONCURRENT);
        }
    });
...
- (void)logInfo:(NSString*)module logStr:(NSString*)logStr, ...{
    ...
    //写入日志
    // 异步执行
//    dispatch_async(dispatch_queue_create("CIBN_WriteLogQueue", nil), ^{
//        // 获取当前日期做为文件名
...
//
//        // [时间]-[模块]-日志内容
//        NSString* timeStr = [self.timeFormatter stringFromDate:[LRZLoggerManager getCurrDate]];
...
- (void)clearExpiredLog{
    ...
    for (NSString* file in files) {
        ...
        if (date) {
            ...
            NSTimeInterval currTime = [[LRZLoggerManager getCurrDate] timeIntervalSince1970];
